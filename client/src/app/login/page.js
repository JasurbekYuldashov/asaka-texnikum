			...
			<div className="w-full m-2 mt-8 shadow dark:border sm:max-w-md  dark:bg-gray-800 dark:border-gray-700 rounded-lg backdrop-blur-lg shadow-blue-950 shadow-4xl">
				<div className="p-6 space-y-4 md:space-y-6 sm:p-8">
					<h1 className="text-xl font-bold leading-tight tracking-tight text-gray-900 md:text-2xl dark:text-white">
						{LogSign ?
							"Ro'yxatdan o'tish (Hisob yaratish)" : 'Hisobingizga kirish'
						}
					</h1>
					<div className="space-y-3 md:space-y-4" >
						{LogSign &&
							<div>
								<label htmlFor="Name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Ism</label>
								<input type="text" name="Name" id="Name" className={InputFieldClass} placeholder="Ismingizni kiriting" required="" value={Name} onChange={(e) => setName(e.target.value)} />
							</div>
						}
						{LogSign &&
							<div>
								<label htmlFor="UserName" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Foydalanuvchi nomi</label>
								<input type="text" name="UserName" id="UserName" className={InputFieldClass} placeholder="Foydalanuvchi nomini kiriting" required="" value={UserName} onChange={(e) => setUserName(e.target.value)} />
							</div>
						}
						<div>
							<label htmlFor="Email" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email</label>
							<input type="email" name="Email" id="Email" className={InputFieldClass} placeholder="Email manzilni kiriting" required="" value={Email} onChange={(e) => setEmail(e.target.value)} />
						</div>
						{LogSign &&
							<div>
								<label htmlFor="Contact" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Telefon</label>
								<input type="text" name="Contact" id="Contact" className={InputFieldClass} placeholder="Telefon raqamingizni kiriting" required="" value={Contact} onChange={(e) => setContact(e.target.value)} />
							</div>
						}
						<div>
						...
						{/* <div className="flex items-center justify-between">
							<div className="flex items-start">
								...
								<div className="ml-3 text-sm">
									<label htmlFor="remember" className="text-gray-500 dark:text-gray-300">Meni eslab qolish</label>
								</div>
							</div>
							{Email && <>
								{PassResetLoading ? <ColorRingLoader /> :
									<button type="submit" className="font-bold underline  hover:text-green-700 dark:text-purple-500 text-purple-900">Parolni tiklash</button>
								}
							</>}
						</div> */}
						...
						<p className="text-sm text-gray-900 dark:text-gray-400">
							{LogSign ?
								'Hisobingiz bormi? ' : 'Hisobingiz yo\'qmi? '
							}
							<button className="font-bold underline hover:text-green-700 dark:text-purple-500 text-purple-900" onClick={() => { setLogSign((prev) => !prev) }}>
								{LogSign ?
									'Kirish' : 'Ro\'yxatdan o\'tish (hisob yaratish)'
								}
							</button>
						</p>
					</div>
